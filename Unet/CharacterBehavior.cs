...
public class CharacterBehavior : NetworkBehaviour,CanTakeDamage
{
    ...
    [ClientRpc]
    public void Rpcincastle()
    {
        incastlecure();
    }

    private void incastlecure()
    {
        Health += 1;
        if (Health >= BehaviorParameters.MaxHealth)
        {
    ...
    [ClientRpc]
    public void Rpcinbigmonster()
    {
        
        StartCoroutine(WaitBigHurtAgain());
        Health -= 1;
        if (Health <= 1)
        {
            Health = 1;
    ...
    [Client]
    public void RpcInSmallMonster()
    {
        StartCoroutine(WaitHurtAgain());
        Health -= 1;
        if (Health <= 1)
        {
            Health = 1;
    ...
    IEnumerator WaitHurtAgain()
    {
        yield return new WaitForSeconds(60);
    }
    IEnumerator WaitBigHurtAgain()
    {
